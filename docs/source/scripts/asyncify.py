...
### Code to translate simple python code to be async. n.b. right now only sleep
### calls and imports are async in practice
# all calls to local functions are async as otherwise you can't run sleep in
# them
...
class _MakeAsyncCalls(NodeTransformer):
    ...
    def visit_ImportFrom(self, node):
        ...
        # first call async code to import it into pyodide, then call the
        # original import statement to make it be available here
        newNode = [
            Expr(
                value=Await(
                    Call(
                        Name("aimport", ctx=Load()),
                        args=[List(elements, ctx=Load())],
                        keywords=[],
                    )
                )
            ),
    ...
    def visit_FunctionDef(self, node):
        ...
        self.generic_visit(
            node
        )  # make sure any calls are turned into awaits where relevant
...
# todo make this for multiple code modules (and maybe to guess class types from
# the code..)
